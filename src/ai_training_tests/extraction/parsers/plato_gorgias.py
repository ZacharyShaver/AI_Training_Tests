...
DEFAULT_SOURCE = REPO_ROOT / "source_texts/occult/raw/plato_gorgias_jowett.txt"
...
SOCRATES = "Socrates"
INTERLOCUTORS = {"Callicles", "Polus", "Gorgias", "Chaerephon"}

SPEAKER_RE = re.compile(
    r"^(?P<label>SOCRATES|CALLICLES|POLUS|GORGIAS|CHAEREPHON)"
    r"[.:\s]?\s*(?P<text>.*)$"
)
NOISE_RE = re.compile(
    r"^(introduction|INTRODUCTION|[IVX]+\.\s|THE\s+ARGUMENT|SCENE:|chapter\s+\d)",
    re.IGNORECASE,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Parse Plato Gorgias dialogue rows.")
    parser.add_argument("--source", type=Path, default=DEFAULT_SOURCE)
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_OUTPUT_DIR)
    parser.add_argument("--dataset-name", default="plato_gorgias_dialogue")
    ...
    review_path.write_text(
        "\n".join(
            [
                "# Plato Gorgias Dialogue Review Sample",
                "",
