...
from ai_training_tests.extraction.common.text_cleaning import count_dataset_words
...
def target_is_fragmentary(text: str) -> bool:
    ...
    if stripped.count('"') % 2 == 1:
        return True
    if stripped.count("\u00e2\u20ac\u0153") != stripped.count("\u00e2\u20ac\u009d"):
        return True
    return False
...
def auto_hard_failures(row: dict) -> list[str]:
    ...
    if not isinstance(target_participant, str) or not target_participant.startswith(
        "Participant B"
    ):
        failures.append(HARD_FAILURE_BROKEN_MAPPING)
    if parsed.instruction_participant and not parsed.instruction_participant.startswith(
        "Participant B"
    ):
        failures.append(HARD_FAILURE_BROKEN_MAPPING)
