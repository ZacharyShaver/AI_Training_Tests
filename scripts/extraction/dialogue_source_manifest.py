#!/usr/bin/env python3
"""Approved direct-source dialogue rebuild manifest."""
...
def source_labels_for_family(family: str) -> set[str]:
    return {
        spec.source_label for spec in APPROVED_SOURCE_SPECS if spec.family == family
    }


def specs_for_output_group(output_group: str) -> list[SourceBuildSpec]:
    return [
        spec for spec in APPROVED_SOURCE_SPECS if spec.output_group == output_group
    ]
